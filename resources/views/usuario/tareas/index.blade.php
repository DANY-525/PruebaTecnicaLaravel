				...
				<table class="table table-striped table-bordead table-condensed table-hover">
					...
					@foreach($tareas as $task)
						...
						<tr>
							...
							<td>
								<a href="{{URL::action('UserTareasController@edit',$task->Idtask)}}"> <button class="btn btn-info">Editor </button></a>
								<a href="" data-target="#modal-delete-{{$task->Idtask}}" data-toggle="modal"> <button class="btn btn-danger">Eliminar</button></a>


								<a href="{{URL::action('UserComentariosController@show',$task->Idtask,$task->FK_UserName)}}"> <button class="btn btn-info">Comentar </button></a>
